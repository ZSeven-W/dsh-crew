...
function workspaceKey(cwd) {
	let h = 2166136261;
	const s = cwd == null || cwd === "" ? "\0" : String(cwd);
...
function groupJobs(rows) {
	...
	const groups = [...byCwd.entries()].map(([cwd, jobs]) => ({
		cwd,
		jobs: sortJobs(jobs)
	})).sort((a, b) => a.cwd.localeCompare(b.cwd));
	if (none.length > 0) groups.push({
		cwd: null,
		jobs: sortJobs(none)
	});
	return groups;
...
function lockAge(startedAt, now) {
	if (startedAt === void 0 || startedAt === null || startedAt === "") return "";
	const s = Math.max(0, Math.round(((now ?? Date.now()) - +new Date(startedAt)) / 1e3));
...
*/
function lockBadgeOf(locks, cwd, copy, now) {
...
*/
function ghostPidOf(job) {
	if (!job || typeof job !== "object") return null;
	const pid = job.pid;
	if (!Number.isInteger(pid) || pid <= 0) return null;
	const pgid = job.pgid;
	if (pgid === void 0 || pgid === null) return {
		pid,
		pgid: null
	};
...
*/
function procBadgeOf(job, probe) {
...
*/
function dueProcProbes(ghosts, probedAt, now) {
	const out = [];
	for (const [id, g] of Object.entries(ghosts ?? {})) {
		...
		if (Number.isFinite(at) && now - at < 1e4) continue;
		out.push({
			id,
			...p
		});
...
function readCollapsed(key, defaultCollapsed = false) {
	let stored = null;
	try {
...
function persistCollapsed(key, collapsed) {
	try {
		...
		collapseSessionMemory[key] = collapsed;
	}
}
...
function useCollapseSection(section, defaultCollapsed = false) {
	const [collapsed, setCollapsed] = (0, react.useState)(() => readCollapsed(COLLAPSE_PREFIX + section, defaultCollapsed));
...
function useWorkspaceCollapse(wsKey) {
	const [collapsed, setCollapsed] = (0, react.useState)(() => readCollapsed(WS_COLLAPSE_PREFIX + wsKey));
	return [collapsed, (0, react.useCallback)(() => {
		const next = !collapsed;
		setCollapsed(next);
...
const COPY = {
	zh: {
		...
		procAliveBadgeTip: (pid) => `任务已失联，但其工作进程仍在运行（pid ${pid}）：很可能是派活宿主崩溃后留下的孤儿进程`,
		procKillTip: (pid) => `杀死 pid ${pid}：先读取命令行校验确为 dsh-crew 工作进程（DSH 运行时 / agy / grok），再 SIGTERM，约 3 秒后仍存活则 SIGKILL`,
		finishedJobs: "已结束",
		noCwdGroup: "（未指定工作区）",
		lockBadgeTip: (h) => `「${h?.cwd ?? "此工作区"}」正被运行中任务持锁（job ${h?.jobId} · backend ${h?.backend} · 开始于 ${h?.startedAt}）：对该 cwd 的再次派发会被拒绝，除非传 allow_concurrent_cwd: true。`,
	...
	en: {
		...
		procAliveBadgeTip: (pid) => `The job lost contact with the board, but its worker process is still running (pid ${pid}) - most likely an orphan left behind after its dispatching host died`,
		procKillTip: (pid) => `Kill pid ${pid}: the command line is read and verified first (DSH runtime / agy / grok), then SIGTERM, and SIGKILL if it survives ~3s`,
		finishedJobs: "Finished",
		noCwdGroup: "(no workspace)",
		lockBadgeTip: (h) => `"${h?.cwd ?? "this workspace"}" is held by a running job (job ${h?.jobId} · backend ${h?.backend} · started ${h?.startedAt}): a second dispatch to this cwd is refused unless allow_concurrent_cwd: true is passed.`,
...
function WorkersPanel({ ctx }) {
	...
	const [procProbe, setProcProbe] = (0, react.useState)({});
	const procProbeAt = (0, react.useRef)({});
	const [killing, setKilling] = (0, react.useState)({});
	...
	const [presetRoster, setPresetRoster] = (0, react.useState)({ presets: [] });
	const presetOptions = (0, react.useMemo)(() => [{
		value: "default",
		label: copy.presetDefault(presetRoster.defaultId)
	...
	* (DSH's setting may be unset), and the hub localizes its own strings from it. */
	...
	const probeOne = (0, react.useCallback)(async (id, pid) => {
		let alive = null;
		try {
	...
	(0, react.useEffect)(() => {
		const run = () => {
			...
			if (targets.length > 0) Promise.all(targets.map((t) => probeOne(t.id, t.pid)));
		};
		run();
		const timer = setInterval(run, 5e3);
		return () => clearInterval(timer);
	}, [ghosts, probeOne]);
	const killGhostProc = (0, react.useCallback)(async (id) => {
		const g = ghosts[id];
	...
	const refreshAll = (0, react.useCallback)(async () => {
		try {
			...
			if (pr.ok) setPresetRoster({
				defaultId: pr.defaultId,
				presets: pr.presets ?? []
			});
		} catch {}
	}, [get, mergeJobs]);
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("td", {
					...
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							children: job.task
						}), pb && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							title: copy.procAliveBadgeTip(pb.pid),
							style: {
								flexShrink: 0,
								fontSize: 11,
								color: "#f0883e",
							...
							children: copy.procAliveBadge
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							disabled: !!killing[job.id],
							title: copy.procKillTip(pb.pid),
							style: {
								flexShrink: 0,
								fontSize: 11,
								padding: "0 8px",
								borderRadius: 99,
								border: "1px solid rgba(240,136,62,0.65)",
								background: "transparent",
								color: "#f0883e",
								cursor: "pointer",
								whiteSpace: "nowrap"
							...
							onClick: (e) => {
								e.stopPropagation();
								killGhostProc(job.id);
							},
							children: killing[job.id] ? copy.procKilling : copy.procKill
						})] })]
					})
				})
			]
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [
			...
			!integrationsCollapsed && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					integrationRow("Claude Code", !!status?.claude?.installed, status?.claude?.installed ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						style: S.chip(!!status?.claude?.hud),
						children: copy.hud
					}) : null, "claude", "claude-uninstall", copy.tips.claude),
					integrationRow("Codex", !!status?.codex?.installed, null, "codex", "codex-uninstall", copy.tips.codex),
					integrationRow("Antigravity", !!status?.agy?.installed, null, "agy", "agy-uninstall", copy.tips.agy),
					integrationRow("Grok", !!status?.grok?.installed, null, "grok", "grok-uninstall", copy.tips.grok)
				]
			}),
			...
			!jobsLoaded || groups.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				style: { opacity: .55 },
				children: copy.empty
			}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("table", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("tbody", { children: groups.map((group) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(WorkspaceGroup, {
							group,
							locks,
							copy,
							renderRow: renderJobRow
						}, workspaceKey(group.cwd))) })
					]
				})
			}),
...
function WorkspaceGroup({ group, locks, copy, renderRow }) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("tr", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("td", {
		colSpan: 7,
		role: "button",
		tabIndex: 0,
		"aria-expanded": !collapsed,
		...
		children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			...
			children: [
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					...
					children: group.cwd === null ? copy.noCwdGroup : basenameOf(group.cwd)
				}),
				group.cwd !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					title: group.cwd,
					style: {
						...S.mono,
						fontSize: 11,
		...
		finished.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("tr", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("td", {
			colSpan: 7,
			role: "button",
			tabIndex: 0,
