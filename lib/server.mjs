...
async function startJob({ task, tier = "flash", effort = "max", cwd, maxTokens = 49152, timeoutMs = 18e5, source = "api" }) {
...
import { readFileSync as readFileSync3, writeFileSync as writeFileSync2, mkdirSync as mkdirSync3, copyFileSync, existsSync as existsSync2, readdirSync as readdirSync2, rmSync as rmSync2 } from "node:fs";
import { dirname as dirname2, join as join3, resolve as resolve2, relative } from "node:path";
import { fileURLToPath as fileURLToPath2 } from "node:url";
import { homedir as homedir3 } from "node:os";
var ROOT2 = resolve2(dirname2(fileURLToPath2(import.meta.url)), "..", "..");
...
var server = new McpServer({ name: "dsh-crew", version: "0.1.0-rc.3" });
var tierSchema = external_exports.enum(["flash", "pro"]).optional().describe("Worker model tier: flash = deepseek-v4-flash (simple tasks), pro = deepseek-v4-pro (harder tasks). Omit to use the session default.");
var effortSchema = external_exports.enum(["off", "high", "max"]).optional().describe("Reasoning effort for the worker. Omit to use the session default.");
...
server.registerTool("dsh_run_worker", {
  ...
  const runOnce = async (t) => {
    ...
    const job2 = await startJob({ task, tier: t, effort: e, cwd: workDir, timeoutMs: timeout * 1e3, source: ORCHESTRATOR });
    await waitJob(job2.id, timeout * 1e3);
    return jobView(job2, { withResult: true });
  };
