...
async function startJob({ task, tier = "flash", effort = "max", cwd, maxTokens = 49152, timeoutMs = 18e5, source = "api", origin = null, allowConcurrentCwd = false }) {
...
server.registerTool("dsh_run_worker", {
  ...
  const runOnce = async (t) => {
    if (await resolveMode() === "hub") {
      ...
      let spawned;
      try {
        spawned = await hub.spawn({ task, tier: t, effort: e, cwd: workDir, source: ORCHESTRATOR, preset: presetForTier(t), origin_chain: origin2.origin.chain, origin_depth: origin2.origin.depth });
    ...
    let job2;
    try {
      job2 = await startJob({ task, tier: t, effort: e, cwd: workDir, timeoutMs: timeout * 1e3, source: ORCHESTRATOR, origin: origin.origin, allowConcurrentCwd: !!allow_concurrent_cwd });
    } catch (err) {
      if (err instanceof CwdLockError) return { refusal: text({ error: err.message, rejected_by: "cwd-lock", holder: err.holder }) };
      throw err;
    }
