...
// src/jobs.mjs
import { readFileSync as readFileSync2, mkdirSync as mkdirSync2, existsSync } from "node:fs";

// src/status-shard.mjs
import { writeFileSync, mkdirSync, rmSync, readdirSync, readFileSync } from "node:fs";
import { join } from "node:path";
import { homedir } from "node:os";
var CONFIG_DIR = join(homedir(), ".config", "dsh-crew");
var SHARD_DIR = join(CONFIG_DIR, "status.d");
var SHARD_FRESH_MS = 30 * 60 * 1e3;
function createShardWriter(kind) {
  ...
  return {
    ...
    publish(jobs2) {
      try {
        ...
        writeFileSync(file, JSON.stringify({ updatedAt: (/* @__PURE__ */ new Date()).toISOString(), writer, jobs: jobs2 }, null, 2));
      } catch {
      }
    },
    dispose: cleanup
...
function normalizeCwd(cwd) {
  try {
    return realpathSync.native(String(cwd));
  } catch {
    return resolve(String(cwd));
  }
}
var CwdLockError = class extends Error {
  constructor(holder) {
    super(
      `workspace "${holder.cwd}" is already held by a running worker (job ${holder.jobId}, backend ${holder.backend}, mode ${holder.mode}, started at ${holder.startedAt}). Refusing this dispatch instead of queueing it: two workers writing the same repo concurrently corrupt files. Wait for that job to settle, cancel it, or pass allow_concurrent_cwd: true if this task is read-only.`
    );
    this.name = "CwdLockError";
    this.holder = { ...holder };
  }
};
function acquireCwdLock({ cwd, jobId, backend, mode, startedAt, allowConcurrent = false }) {
...
function updateCwdLockHolder({ cwd, jobId }) {
  const holder = locks.get(normalizeCwd(cwd));
  if (holder) holder.jobId = jobId;
}
function releaseCwdLockByJobId(jobId) {
...
function getCwdLocks() {
  return [...locks.entries()].map(([cwd, holder]) => ({ cwd, holder: { ...holder } }));
}

// src/origin-guard.mjs
var ORIGIN_CHAIN_ENV = "DSH_CREW_ORIGIN_CHAIN";
var ORIGIN_DEPTH_ENV = "DSH_CREW_DEPTH";
var DEFAULT_ORIGIN_DEPTH_LIMIT = 3;
function readInheritedOrigin(env = process.env) {
...
function formatChain(chain) {
  return chain.map((h, i) => `  ${i + 1}. ${h.source ?? "unknown-host"} \u2192 ${h.backend} @ ${h.cwd}`).join("\n");
}
function extendOrigin({ inherited, backend, cwd, source = null, depthLimit = DEFAULT_ORIGIN_DEPTH_LIMIT }) {
  const hop = { backend, cwd: normalizeCwd(cwd), source };
  const chain = [...inherited.chain, hop];
  const depth = chain.length;
  const seen = /* @__PURE__ */ new Map();
  for (const h of chain) {
    const key = h.backend + "\0" + h.cwd;
    ...
    if (first) {
      return {
        ...
        error: {
          ...
          message: `Worker self-dispatch loop detected \u2014 refusing to dispatch.
The hop ${backend} @ ${hop.cwd} repeats an earlier hop of the origin chain; dispatching it would close a worker\u2192worker cycle, the classic recursive self-amplification (quota burns in minutes, invisible from any single vantage point).
  ...
  if (depth > depthLimit) {
    return {
      ...
      error: {
        ...
        message: `Origin chain depth ${depth} exceeds the configured limit of ${depthLimit} \u2014 refusing to dispatch.
This dispatch would nest workers ${depth} levels deep; beyond the limit, nesting is almost certainly workers spawning workers (recursive self-amplification).
Origin chain (source \u2192 backend @ cwd):
${formatChain(chain)}
...
var CONFIG_DIR2 = join2(homedir2(), ".config", "dsh-crew");
var STATUS_FILE = join2(CONFIG_DIR2, "status.json");
var RUNTIME_BIN = join2(ROOT, "node_modules", ".bin", "dsh-jsonrpc-agent");
var CORDIS = join2(ROOT, "worker.cordis.yml");
var TIERS = {
...
function loadDotEnv() {
  const out = {};
  try {
    for (const line of readFileSync2(join2(CONFIG_DIR2, ".env"), "utf8").split("\n")) {
...
var nextId = 1;
var shard = createShardWriter("mcp");
function backendOf(j) {
  return j.backend ?? "standalone";
}
function profileOf(j) {
  return j.profile ?? null;
}
function modeOf(j) {
  return j.mode ?? (j.backend ? "cli" : "standalone");
}
...
function publishStatus() {
  shard.publish([...jobs.values()].map((j) => ({
    ...
    status: j.status,
    source: j.source,
    task: clipTask(j.task),
    cwd: j.cwd,
    turn: j.turn,
    step: j.step,
    toolCalls: j.toolCalls,
    currentTool: j.currentTool,
    tokens: j.tokens,
    backend: backendOf(j),
    profile: profileOf(j),
    mode: modeOf(j),
    ...
    pid: j.pid ?? null,
    pgid: j.pgid ?? null,
    origin_depth: j.origin_depth ?? null,
    origin_chain: j.origin_chain ?? null,
    startedAt: j.startedAt,
    endedAt: j.endedAt
  })));
}
function jobView(j, { withResult = false } = {}) {
  const v = {
    id: j.id,
    tier: j.tier,
    model: j.model,
  ...
  if (j.activity) {
    v.activity = {
      ...
      staleSeconds: j.activity.lastEventAt ? Math.round((Date.now() - Date.parse(j.activity.lastEventAt)) / 1e3) : null
    };
...
async function startJob({ task, tier = "flash", effort = "max", cwd, maxTokens = 49152, timeoutMs = 18e5, source = "api", origin = null, allowConcurrentCwd = false, provider, model }) {
  ...
  const workspace = resolve2(cwd ?? process.cwd());
  ...
  const job = {
    ...
    startedAt,
    endedAt: null,
    origin_depth: origin?.depth ?? null,
    origin_chain: origin?.chain ?? null,
    result: null,
    error: null,
    stopReason: null,
    harness: null,
  ...
  try {
    ...
    if (!process.env.DEEPSEEK_API_KEY && !dotEnv.DEEPSEEK_API_KEY) {
      throw new Error(`DEEPSEEK_API_KEY not found in env or ${join2(CONFIG_DIR2, ".env")}`);
    }
    const harness = new DeepSeekHarness({
      launch: {
        ...
        env: {
          ...
          ...dotEnv,
          DSH_CWD: workspace,
          DSH_SESSION_ROOT: join2(CONFIG_DIR2, "sessions"),
    ...
    const onNotification = (n) => {
      if (job.status === "running") {
        ...
        if (child?.pid && job.pid !== child.pid) {
          job.pid = child.pid;
          publishStatus();
        }
      }
      ...
      switch (e.type) {
        case "step/start":
          job.turn = e.data?.turn ?? job.turn;
          job.step = e.data?.step ?? job.step;
          break;
        case "tool/call":
          job.currentTool = e.data?.name ?? null;
          job.toolCalls += 1;
          break;
        case "tool/result":
        ...
        case "turn/end":
          job.stopReason = e.data?.reason?.kind ?? null;
          break;
      }
      publishStatus();
    };
    job.promise = harness.run(task, { sessionId, onNotification }).then((result) => {
...
import { readFileSync as readFileSync3, writeFileSync as writeFileSync2, mkdirSync as mkdirSync3, copyFileSync, existsSync as existsSync2, readdirSync as readdirSync2, rmSync as rmSync2, rmdirSync } from "node:fs";
import { dirname as dirname2, join as join3, resolve as resolve3, relative } from "node:path";
import { fileURLToPath as fileURLToPath2 } from "node:url";
import { homedir as homedir3 } from "node:os";
var ROOT2 = resolve3(dirname2(fileURLToPath2(import.meta.url)), "..", "..");
var MARKETPLACE_NAME = "dsh-crew";
var PLUGIN_KEY = `dsh-crew@${MARKETPLACE_NAME}`;
...
try {
  const oldDir = join3(homedir3(), ".config", "dsh-workers");
  const newDir = join3(homedir3(), ".config", "dsh-crew");
...
var GLOBAL_CONFIG_FILE = join3(CONFIG_DIR_HOME(), "config.json");
function CONFIG_DIR_HOME() {
  return join3(homedir3(), ".config", "dsh-crew");
}
var GLOBAL_CONFIG_DEFAULTS = {
  ...
  // claude-code | codex | grok | agy | <custom id> | off
  vision_model: "haiku",
  // model passed to the vision CLI; free-form
  imagegen_provider: "codex",
  // codex | agy | grok | <custom id> | off
...
// src/plugin-env.mjs
var PLUGIN_ENV_PREFIX = "DSHPLUGIN_";
var LEGACY_ENV_PREFIX = "DSH_";
var PLUGIN_LABEL = "dsh-crew";
var warned = /* @__PURE__ */ new Set();
function pluginEnvName(suffix) {
  return PLUGIN_ENV_PREFIX + suffix;
}
function legacyEnvName(suffix) {
  return LEGACY_ENV_PREFIX + suffix;
}
function pluginEnv(suffix, options = {}) {
  ...
  const hasCurrent = env[current] !== void 0;
  const hasLegacy = env[legacy] !== void 0;
  if (hasLegacy && !warned.has(legacy)) {
    warned.add(legacy);
    warn(
      `${PLUGIN_LABEL}: ${legacy} is deprecated; rename it to ${current}` + (hasCurrent ? ` (${current} is also set and takes precedence)` : "") + `. DSH reserves the ${LEGACY_ENV_PREFIX} prefix for itself: a .env file that sets ${legacy} makes the host abort at startup, so REPLACE the old assignment \u2014 adding the new name beside it does not help.`
    );
  }
...
// src/hub-client.mjs
var BASE = (pluginEnv("CREW_HUB") ?? readGlobalConfig().hub_url).replace(/\/$/, "");
var API = `${BASE}/_dsh/dsh-crew`;
var lastProbe = { at: 0, ok: false };
async function hubAvailable() {
  if (Date.now() - lastProbe.at < 1e4) return lastProbe.ok;
  try {
...
var WORKER_PROFILES = {
  ...
  agy: {
    name: "agy",
    backend: "agy",
    // backend key implemented in cli-workers.mjs
    cli: "agy",
    // binary resolved from PATH at spawn time
...
// src/cli-workers.mjs
var STALL_SECONDS = 120;
var DEFAULT_TIMEOUT_MS = 18e5;
var SIGKILL_GRACE_MS = 2e3;
var PUBLISH_MIN_INTERVAL_MS = 2e3;
var RAW_LINE_CAP = 300;
...
function buildArgs(profile, task, timeoutMs, effortOverride) {
  if (profile.backend === "agy") {
    ...
    const args = [
      ...
      "--print-timeout",
      `${timeoutSec + 15}s`
      // self-exit backstop above our deadline
    ];
...
function handleGrokLine(job, line, msg) {
  ...
  if (msg.type === "stream_event") {
    ...
    switch (e.type) {
      ...
      case "message_delta": {
        if (e.usage) job._msgUsage = e.usage;
  ...
  if (msg.type === "assistant" && msg.message) {
    const m = msg.message;
    const text2 = (m.content ?? []).filter((c) => c?.type === "text").map((c) => c.text).join("");
    if (text2) job._msgText = text2;
  ...
  if (msg.type === "result") {
    ...
    if (msg.usage) setTokensFromUsage(job, msg.usage);
    if (msg.is_error) {
...
function settle(job, { status, error, stopReason }) {
  ...
  if (job.status === "cancelled") {
    ...
    job.status = status;
    if (error !== void 0) job.error = job.error ?? error;
    if (stopReason !== void 0) job.stopReason = job.stopReason ?? stopReason;
  }
  job.endedAt = (/* @__PURE__ */ new Date()).toISOString();
  publishStatus();
  for (const w of job.waiters.splice(0)) w();
}
async function startCliJob({ worker, task, cwd, timeoutMs = DEFAULT_TIMEOUT_MS, source = "api", effort, origin = null, allowConcurrentCwd = false }) {
  const profile = typeof worker === "string" ? resolveProfile(worker) : worker;
  const workspace = resolve4(cwd ?? process.cwd());
  ...
  const job = {
    ...
    mode: "cli",
    tier: profile.name,
    // display: profile name rides the tier column
  ...
  job._timer = setTimeout(() => {
    ...
    job.error = `timed out after ${Math.round(timeoutMs / 1e3)}s (CLI process group killed)`;
    job.liveness = "stalled";
    publishStatus();
    killGroup(child);
    job._timedOut = true;
...
// src/tier-binding.mjs
var DEFAULT_TIER_BINDING = Object.freeze({
...
var TIERS2 = Object.freeze(["flash", "pro"]);
function configured(value) {
  return typeof value === "string" && value.trim() !== "" ? value.trim() : void 0;
}
function resolveTierBinding(tier, config = {}) {
  const fallback = DEFAULT_TIER_BINDING[tier];
  if (fallback === void 0) {
    throw new Error(`unknown tier "${tier}" (expected: ${TIERS2.join(", ")})`);
  }
  return {
...
function isCustomBinding(tier, config = {}) {
...
var server = new McpServer({ name: "dsh-crew", version: "0.1.0-rc.10" });
var tierSchema = external_exports.enum(["flash", "pro"]).optional().describe("Worker model tier. flash = mechanical, well-scoped work (single-file edits, lookups, formatting); pro = multi-file changes, debugging, design judgment. Omit to use the session default. Ignored when worker= is set.");
var effortSchema = external_exports.enum(["off", "high", "max"]).optional().describe("Reasoning effort for the worker. Omit to use the session default. With worker= it only applies when passed explicitly and the profile supports it (agy maps to low/medium/high).");
var workerSchema = external_exports.string().optional().describe('Named CLI worker profile (e.g. "agy" or "grok"). Explicit opt-in only: set it when the user explicitly asks for that external coding CLI, never as a default. When set, dispatch bypasses tier/mode and runs the task through that CLI; the profile pins backend\xD7model\xD7effort. See dsh_worker_config output, worker_profiles field, for the available profiles.');
var allowConcurrentCwdSchema = external_exports.boolean().optional().describe("Allow this dispatch even though another running worker already holds the same cwd (workspace) lock. Default false: concurrent writers corrupt a shared repo, so the second dispatch is refused with the holder info instead of queueing. Set true only for read-only tasks.");
var globalDefaults = readGlobalConfig();
var sessionConfig = {
  enabled: true,
  ...
  tier_policy: globalDefaults.tier_policy,
  // auto | flash-only | pro-only
...
var INHERITED_ORIGIN = readInheritedOrigin(process.env);
var depthLimitNow = () => sessionConfig.origin_depth_limit ?? DEFAULT_ORIGIN_DEPTH_LIMIT;
var hubOrigins = /* @__PURE__ */ new Map();
function attachOrigin(view, jobId) {
  if (!view || typeof view !== "object") return view;
  const o = hubOrigins.get(jobId);
  if (o && view.origin_depth === void 0) {
...
async function acquireDispatchLock({ cwd, backend, mode, allowConcurrent }) {
  for (let attempt = 0; ; attempt++) {
    try {
      return acquireCwdLock({ cwd, jobId: null, backend, mode, startedAt: (/* @__PURE__ */ new Date()).toISOString(), allowConcurrent });
    } catch (err) {
      if (!(err instanceof CwdLockError) || attempt > 0 || err.holder.mode !== "hub") throw err;
      let settled = false;
      try {
        settled = (await hub.get(err.holder.jobId, 0)).status !== "running";
      } catch (e) {
        if (String(e?.message ?? e).includes("no such job")) settled = true;
      }
...
server.registerTool("dsh_run_worker", {
  ...
  inputSchema: {
    task: external_exports.string().describe("Full task description for the worker, self-contained"),
    tier: tierSchema,
    effort: effortSchema,
    worker: workerSchema,
  ...
  if (worker) {
    ...
    let job2;
    try {
      job2 = await startCliJob({ worker, task, cwd: workDir, timeoutMs: timeout * 1e3, source: ORCHESTRATOR, effort, origin: origin.origin, allowConcurrentCwd: !!allow_concurrent_cwd });
    } catch (err) {
      if (err instanceof CwdLockError) return text({ error: err.message, rejected_by: "cwd-lock", holder: err.holder });
      throw err;
    }
    await waitJob(job2.id, timeout * 1e3);
    const view = jobView(job2, { withResult: true });
  ...
  const runOnce = async (t) => {
    if (await resolveMode() === "hub") {
      const origin2 = extendOrigin({ inherited: INHERITED_ORIGIN, backend: "hub", cwd: workDir, source: ORCHESTRATOR, depthLimit });
      if (!origin2.ok) return { refusal: originRefusal(origin2) };
      let lock;
      try {
      ...
      let spawned;
      try {
        spawned = await hub.spawn({ task, tier: t, effort: e, cwd: workDir, source: ORCHESTRATOR, preset: presetForTier(t), ...bindingForTier(t), origin_chain: origin2.origin.chain, origin_depth: origin2.origin.depth });
        updateCwdLockHolder({ cwd: workDir, jobId: spawned.id });
        hubOrigins.set(spawned.id, origin2.origin);
      } catch (err) {
        lock.release();
        throw err;
      }
    ...
    let job2;
    try {
      job2 = await startJob({ task, tier: t, effort: e, cwd: workDir, timeoutMs: timeout * 1e3, source: ORCHESTRATOR, origin: origin.origin, allowConcurrentCwd: !!allow_concurrent_cwd, ...bindingForTier(t) });
    } catch (err) {
      if (err instanceof CwdLockError) return { refusal: text({ error: err.message, rejected_by: "cwd-lock", holder: err.holder }) };
      throw err;
    }
    await waitJob(job2.id, timeout * 1e3);
    return { view: jobView(job2, { withResult: true }) };
  };
  ...
  if (job.status === "failed" && sessionConfig.escalate_on_failure && firstTier === "flash" && sessionConfig.tier_policy !== "flash-only") {
...
server.registerTool("dsh_worker_config", {
  ...
  inputSchema: {
    enabled: external_exports.boolean().optional().describe("false = refuse all worker dispatch this session"),
    default_tier: external_exports.enum(["flash", "pro"]).optional(),
    default_effort: external_exports.enum(["off", "high", "max"]).optional(),
    mode: external_exports.enum(["auto", "hub", "standalone"]).optional(),
    default_timeout_seconds: external_exports.number().int().positive().max(7200).optional(),
    tier_policy: external_exports.enum(["auto", "flash-only", "pro-only"]).optional().describe("flash-only / pro-only clamp every dispatch to one tier"),
    escalate_on_failure: external_exports.boolean().optional().describe("retry a failed blocking flash run once on pro"),
    preset_flash: external_exports.string().optional().describe('hub-mode agent preset for flash workers (preset id, or "default")'),
    preset_pro: external_exports.string().optional().describe('hub-mode agent preset for pro workers (preset id, or "default")'),
    flash_provider: external_exports.string().optional().describe("LLM provider route the flash tier dispatches to. Must be a provider DSH already has configured \u2014 see available_providers in this tool's output. Empty string restores the built-in DeepSeek default. Configure the provider itself in DSH, not here."),
    flash_model: external_exports.string().optional().describe("Model id for the flash tier, as that provider names it (e.g. a local Ollama model). Empty string restores the default."),
    pro_provider: external_exports.string().optional().describe("LLM provider route the pro tier dispatches to. Same rules as flash_provider."),
    pro_model: external_exports.string().optional().describe("Model id for the pro tier. Empty string restores the default."),
    origin_depth_limit: external_exports.number().int().min(1).max(32).optional().describe("Max worker\u2192worker origin-chain depth (default 3). Deeper dispatches are refused to stop recursive self-amplification; raise only for deliberate deep delegation."),
    reset: external_exports.boolean().optional().describe("true = restore all defaults first")
  }
}, async ({ reset, ...patch }) => {
  if (reset) {
  ...
  for (const [k, v] of Object.entries(patch)) if (v !== void 0) sessionConfig[k] = v;
  ...
  const custom2 = Object.entries(bindings).filter(([, b]) => b.custom).map(([tier]) => tier);
  const unknown = available === null ? [] : custom2.filter((tier) => !available.some((p) => p.id === bindings[tier].provider));
  return text({
    ...sessionConfig,
    hub_reachable: reachable,
    bindings,
    available_providers: available,
    ...available === null && custom2.length > 0 ? { providers_note: "The hub is not reachable, so the configured provider ids could not be checked against what DSH actually has. A route DSH does not know will fail at dispatch, not here." } : {},
    ...unknown.length > 0 ? { providers_warning: `DSH has no provider route named ${unknown.map((t) => JSON.stringify(bindings[t].provider)).join(", ")} (bound to: ${unknown.join(", ")}). Configure it in DSH first, or these dispatches will fail.` } : {},
    ...custom2.length > 0 ? { route_caveat: 'Appearing in available_providers only means an adapter is registered, NOT that it can run a worker. Measured: a route registered purely to re-route another provider fails a dispatch with "registration.adapter.prepareCall is not a function". Nor does DSH report tool-calling support anywhere, and a worker that cannot call tools returns prose and touches no files. A tier bound to a custom route is therefore UNVERIFIED until you dispatch to it once.' } : {},
...
server.registerTool("dsh_spawn_worker", {
  ...
  inputSchema: {
    task: external_exports.string().describe("Full task description for the worker, self-contained \u2014 the worker has no conversation context, so include absolute paths, acceptance criteria and constraints"),
    tier: tierSchema,
    effort: effortSchema,
    worker: workerSchema,
    cwd: external_exports.string().optional().describe(`Workspace directory for the worker. Relative paths resolve against the caller's current workspace; "." or "current" mean the current workspace. Omit for the current workspace.`),
  ...
  if (worker) {
    const profile = resolveProfile(worker);
    const origin2 = extendOrigin({ inherited: INHERITED_ORIGIN, backend: profile.backend, cwd: workDir, source: ORCHESTRATOR, depthLimit });
    if (!origin2.ok) return originRefusal(origin2);
    try {
      const job = await startCliJob({ worker, task, cwd: workDir, source: ORCHESTRATOR, effort, origin: origin2.origin, allowConcurrentCwd: !!allow_concurrent_cwd });
...
server.registerTool("dsh_worker_status", {
  ...
  if (remote !== null) {
    ...
    for (const l of getCwdLocks()) {
      ...
      if (j === void 0 || j.status !== "running") releaseCwdLockByJobId(l.holder.jobId);
    }
  }
  for (const j of remoteJobs) attachOrigin(j, j.id);
  const local = listJobs().map((j) => jobView(j));
  const locks2 = getCwdLocks().map((l, i) => ({
    kind: "cwd-lock",
    id: `cwd-lock-${i + 1}`,
    cwd: l.cwd,
    holder: l.holder,
  ...
  return text([...remoteJobs, ...local, ...locks2]);
});
server.registerTool("dsh_worker_result", {
  ...
  inputSchema: {
    ...
    wait_seconds: external_exports.number().int().min(0).max(7200).default(0).describe("0 = return current state immediately")
  }
  ...
  const job = await waitJob(job_id, wait_seconds > 0 ? wait_seconds * 1e3 : 1);
